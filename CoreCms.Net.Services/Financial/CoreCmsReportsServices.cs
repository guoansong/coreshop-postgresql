...
namespace CoreCms.Net.Services
{
    ...
    public class CoreCmsReportsServices : BaseServices<GetOrdersReportsDbSelectOut>, ICoreCmsReportsServices
    {
        ...
        public List<GetOrdersReportsDbSelectOut> GetOrderMark(int num, string where, int section, DateTime sTime, string joinVal)
        {
            ...
            else if(dbTypeString == DbType.PostgreSQL.ToString())
            {                
                sqlStr = @"SELECT  tmpx.number ,
                                SUM(tmpx.orderAmount) AS val ,
                                COUNT(tmpx.orderId) AS num
                        FROM    ( select * from 
                                ( SELECT 1 as number 
                                UNION SELECT 2 
                                UNION SELECT 3 
                                ...
                                LEFT OUTER JOIN ( SELECT  *
                                                    FROM  CoreCmsOrder 
                                                    WHERE isdel = false " + where + @") o 
                                ON tmp_x.number = EXTRACT(DAY FROM (TIMESTAMP '" + sTime.ToString("yyyy-MM-dd HH:mm:ss") + "'-  o." + joinVal + @"))
                                ) tmpx
                        GROUP BY tmpx.number
                ";
            }
        ...
        public List<GetOrdersReportsDbSelectOut> GetPaymentsMark(int num, string where, int section, DateTime sTime, string joinVal)
        {
            ...
            {                              
                sqlStr = @"SELECT  tmpx.number ,
                                SUM(tmpx.money) AS val ,
                                COUNT(tmpx.paymentId) AS num
                        FROM    ( select * from 
                                ( SELECT 1 as number 
                                UNION SELECT 2 
                                UNION SELECT 3 
